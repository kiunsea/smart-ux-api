@echo off
REM smuxapi-demo WAR 빌드 스크립트
REM Gradle의 war task를 실행하여 WAR 파일을 생성합니다.
...
REM WAR 빌드 프로세스 시작
echo ========================================
echo   smuxapi-demo WAR 빌드 시작
echo ========================================
echo.
echo 작업 디렉터리: %CD%
echo.

REM war task 실행 (smuxapi-demo 디렉터리에서 실행)
call "%GRADLEW%" war
...
echo.
echo ========================================
echo   WAR 빌드가 완료되었습니다!
echo ========================================
echo.
echo WAR 파일 위치:
echo   packaging\distribution\smuxapi.war
echo.
echo 배포 방법:
echo   1. WAR 파일을 Tomcat의 webapps 디렉터리에 복사
echo   2. Tomcat 서버 시작
echo   3. 브라우저에서 http://localhost:8080/smuxapi 접속
echo.
pause

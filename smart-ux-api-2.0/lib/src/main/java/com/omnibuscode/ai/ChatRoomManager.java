...
public class ChatRoomManager {
	...
	private Logger log = LogManager.getLogger(ChatRoomManager.class);
	...
	private ChatRoom chatRoom = null; //TODO 여러 채팅방을 관리하게 변경해야 한다. 그중 ViewChat이 있는거다.(채팅 스레드)
	
	/**
	 * OpenAI OpenAIAssistants API 접속시 필요한 정보 객체(OpenAIAssistant)를 저장한다.
	 * @param aiInfo
	 */
	public void setOpenAIAssistant(Assistant aiInfo) {
	...
	public JSONObject createChatRoom(String aiName) throws Exception {
		...
		if (ChatRoomManager.AI_NAME_OPENAI.equals(aiName)) {
			
		    /**
		     * validation
		     */
			if (this.assistInfo == null) {
				rtnInfo.put("message", "접속 정보(OpenAIAssistants info)가 필요합니다. OpenAIAssistant instance를 설정해 주세요.");
				rtnInfo.put("result", "false");
				return rtnInfo;
			}
		    if (this.assistInfo.getAssistantId() == null) {
		    	rtnInfo.put("message", "OpenAIAssistant_id 가 필요합니다.");
		    	rtnInfo.put("result", "false");
		    	return rtnInfo;
		    }
			...
			this.chatRoom = new OpenAIChatRoom(this.assistInfo);

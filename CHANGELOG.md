...
> **태그 정책**: 리포지토리 태그 (`v0.x.y`) 는 **lib 모듈 릴리스**를 가리키며, 이 태그가 GitHub Release 출시의 단일 트리거입니다.
> smuxapi-demo 는 자체 버전 (`0.10.x`) 을 가지지만 **단독 릴리스/태그는 운영하지 않습니다**. demo 변경은 항상 다음 lib 릴리스에 동반되어 함께 출시됩니다. lib 변경 시 demo 영향을 검토하고, 영향이 있으면 같은 PR/같은 릴리스에 묶어 변경합니다.
...
## smuxapi-demo 변경 이력 (구. 단독 릴리스 운영 — 현재는 lib 동반 정책)

> **정책 변경 안내** — 본 섹션은 demo 가 **lib 태그 없이 단독 릴리스**되던 시기 (v0.10.x 초기) 의 history 입니다. 현재 정책은 "demo 단독 릴리스 금지 — lib 릴리스에 동반 출시" 이며, 향후 demo 변경은 위쪽 `## 활성 릴리스 (태그 보유)` 표의 lib row 에 `lib · demo` 형식으로 함께 기록됩니다. 본 섹션은 신규 row 추가 없이 history 보존 용도로만 유지합니다.
